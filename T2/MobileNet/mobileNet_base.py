...
output_dir = "resultados_mobilenet_base_sin_weights"
...
print("\nIniciando entrenamiento de MobileNet BASE (sin weights, sin dropout)...")
...
model_mobilenet.save(f"{output_dir}/mobilenet_base_sin_weights.keras")
with open(f"{output_dir}/history_mobilenet_base_sin_weights.pkl", "wb") as f:
...
plt.title('MobileNet BASE SIN WEIGHTS - Accuracy')
plt.ylabel('Accuracy')
plt.xlabel('Epocas')
plt.legend()
plt.tight_layout()
...
plt.title('MobileNet BASE SIN WEIGHTS - Loss')
plt.ylabel('Loss')
plt.xlabel('Epocas')
plt.legend()
plt.tight_layout()
...
plt.xlabel("Predicho")
plt.ylabel("Real")
plt.title("Matriz de Confusion - MobileNet BASE SIN WEIGHTS")
